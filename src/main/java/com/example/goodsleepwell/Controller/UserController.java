...
public class UserController {
    ...
    public CompletableFuture<ResponseEntity> downAndUpload(sleepWellBoardContent boardContent) throws ExecutionException, InterruptedException {
        CompletableFuture<Boolean> ret = CompletableFuture.supplyAsync(() -> userService.checkPostorNot(boardContent.getBoardIp()), one).get();
        try {
            log.info("try");
            if (ret.get()) {
    ...
    public CompletableFuture<ResponseEntity> likeUpdate(@Param("id") final int id, @Param("boardIp") final String boardIp) {
        CompletableFuture<Boolean> ret = CompletableFuture.supplyAsync(() -> userService.checkLike(id, boardIp), one).join();
        try {
            if (ret.join()) {
